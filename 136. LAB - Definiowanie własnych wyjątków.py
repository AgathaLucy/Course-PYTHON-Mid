# -*- coding: utf-8 -*-
"""
Created on Sat Jun 20 03:46:20 2020
...
1. Zdefiniuj klasę TripException, która rozszerzy ilość informacji przedstawianych
podczas błędu. Klasa ta będzie przechowywać po prostu dodatkowy atrybut description:
  ...
  metoda __str__ ma zwracać to co zwróciłaby klasa rodzicielska, a dodatkowo 
  informacje zapisane w atrybucie description

2. Zdefiniuj klasę TripNameException dziedziczącą z TripException. Ten rodzaj 
błędu będzie zgłaszany zawsze wtedy, gdy pojawi się problem z nazwą wycieczki. 
Wszystkie te błędy będą miały wspólne description, dlatego zdefiniujemy je na 
stałe w __init__:

  metoda __init__ ma przyjąć tylko argument text

  argument text zostanie przekazany jako pierwszy argument do __init__ klasy 
  rodzicielskiej

  drugi argument może być bardziej rozbudowanym opisem usterki, np. 'Name of the 
  trip is missing. You need to name the trip somehow...'

3. Zdefiniuj klasę TripDateException dziedziczącą z TripException. Ten rodzaj 
błędu będzie zgłaszany zawsze wtedy, gdy pojawi się problem z datami wycieczki. 
Wszystkie te błędy będą miały wspólne description, dlatego zdefiniujemy je na 
stałe w __init__:
  ...
  drugi argument może być bardziej rozbudowanym opisem usterki, np. 'The dates 
  are incorrect. The starting date should be earlier than the ending date...'

4. Zmień wyjątki zgłaszane w metodzie check_data

  w pierwszym przypadku zgłoś TripNameException i skróć tekst przekazywany 
  jako argument

  w drugim przypadku zgłoś TripDateException i też skróć tekst przekazywany 
  jako argument

5. W metodzie publish_offer:
...
class TripException(Exception):
    ...
    def __str__(self):
        return "{}, opis {}".format(super().__str__(), self.description)
